...
```{r Process intergenic HGT output}
...
```{r Assess h-value to use as cutoff}
file3_uniprot %>%
  ggplot(aes(x = h)) +
  stat_ecdf(geom = "point") +
  scale_x_continuous(breaks = c(seq(-200, 300, by = 50), 15, 30)) +
...
```{r Get uniprot hit info AND find prokaryote overlaps}
...
```{r prokaryotic hits to the same gene/organism}
...
```{r prokaryote overlap visualizations}
...
```{r prokaryote hit visualizations}
# file3 %>%
...
```{r sequence similarity ordination prep}
...
```{r Codon usage comparison between uniprot hits}
#
...
```{r Uniprot hits IPG files}
# import files
# acssn_gi <-
#   read_delim("/Users/cbg/01_projects/psammoneis_genome_and_bacteria/data/01_psam_intergenic_HGT_index/hgt_candidates_alignments/protein_ids_summary_GI_grep.txt", delim = " ", col_names = c("accession","gi")) %>% 
#   mutate(gi = str_remove(gi, pattern = "GI:")) %>%
#   mutate(gi = as.numeric(gi))

# IPG <-
#   read_tsv("/Users/cbg/01_projects/psammoneis_genome_and_bacteria/data/01_psam_intergenic_HGT_index/hgt_candidates_alignments/protein_ids.ipg.csv", col_names = TRUE) %>% 
#   clean_names()
# 
# # left join  summary grep to ipg file by GI = ID, then select Nucleotide accession column 
# nt_accsn <-
#   left_join(acssn_gi, IPG, by =c("gi" = "id")) %>%
#   select(accession, nucleotide_accession) %>% 
#   group_by(accession) %>% # group our data by sqeqid
#   slice(1) %>%
#   ungroup() %>% 
#   select(nucleotide_accession) %>%
#   filter(!is.na(nucleotide_accession)) %>% 
#   filter(!nucleotide_accession == 'N/A')
#   
# 
# #export as a space delimited text file
# write_delim(nt_accsn, 
#             file = "/Users/cbg/01_projects/psammoneis_genome_and_bacteria/data/01_psam_intergenic_HGT_index/hgt_candidates_alignments/nt_accessions.delim", 
...
Then, I sorted files to include only lines for orthologs with 'Psammoneis' genes present.

    grep 'Psammoneis' Orthogroups/Orthogroups.tsv > Orthogroups/Orthogroups_Psam_filtered.tsv
...
```{r Crsip HGT index classification}
# HGT index
...
hgt_index_output_orth_prok_psam_classed <-
  hgt_index_output_orth %>%
  filter(!is.na(hgt_class)) %>%
  filter(grepl("^Psammoneisjap", qseqid))

hgt_index_output_orth_euk_psam <-
  hgt_index_output_orth %>%
  filter(domain_assignment == "eukaryote_strong" | domain_assignment == "eukaryote") %>%
  filter(grepl("^Psammoneisjap", qseqid))

hgt_index_output_orth_ind_psam <-
  hgt_index_output_orth %>%
  filter(domain_assignment == "indeterminate") %>%
  filter(grepl("^Psammoneisjap", qseqid))

# Psammoneis subset of hits
 hgt_index_output_orth %>%
  filter(grepl("^Psammoneisjap", qseqid)) %>% 
  select(domain_assignment) %>%
    table(useNA = "ifany")

 # Psammoneis subset of hits, subset to prokaryotic hits and tabled in different ways
    hgt_index_output_orth %>%
  filter(grepl("^Psammoneisjap", qseqid)) %>% 
  filter(domain_assignment == "prokaryote_strong" | domain_assignment == "prokaryote") %>%
...
hgt_index_output_orth_psam %>%
  ungroup() %>% 
  select(protein_names_uniprot_d) %>% 
  unique()

 hgt_index_output_orth_prok %>%
   filter(grepl("^Psammoneisjap", qseqid)) %>% 
  group_by(domain_assignment) %>%
  summarise(cnt = dplyr::n()) %>%
  mutate(freq = round(cnt / sum(cnt), 3)) %>%
...
hgt_index_output_orth %>%
  select(qseqid, bitscore_d, bitscore_r, h, h_orth, domain_assignment, hgt_class, max_bitscore_r_orth) %>% 
  filter(!is.na(hgt_class)) %>% 
    group_by(domain_assignment) %>%
  summarise(cnt = dplyr::n()) %>%
  mutate(freq = round(cnt / sum(cnt), 3)) %>%
...
#   select(length, domain_assignment) %>% 
#   mutate(length = as.numeric(length)) %>% 
#   ggplot(aes(y = domain_assignment, x = length, color = domain_assignment, fill = domain_assignment)) +
...
```{r gff_filter_rename_bac.R, eval=F}
