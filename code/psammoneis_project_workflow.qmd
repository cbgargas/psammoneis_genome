...
# Read in our psammoneis gff file
psam_gff <-
  read_tsv(here("data","01_psam_intergenic_HGT_index","psam_genome","psamm_final.round4.all.gff"), comment = "#", col_names = c("seqname", "source", "feature", "start", "end", "score", "strand", "frame", "attribute")) %>% 
mutate(
  id = case_when(
...
blastp_results_joined <- 
  ...
  # Go thorugh and label NAs with taxonomy categories
  
blastp_results_joined <- 
  left_join(blastp_results_joined,
            (blastp_results_joined %>% 
               group_by(qseqid) %>% 
...
left_join(
  ...
  rename(archaea_blastp_total_hits = Archaea, 
         ...
         top_hit_domain_blastp = top_hit) %>% 
  left_join(., 
            (hgt_index_output_orth_prok_psam %>% 
               select(qseqid, hgt_class, chrm_bac, bac_gen_peg, h, h_orth)),
            by = 'qseqid') 
