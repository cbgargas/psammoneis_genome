...
orthogroups_total <- 
  ...
  as.data.frame() %>% 
  clean_names() %>% 
  mutate(across(2:12, ~ str_count(.x, ',')+1, .names = "{col}_total_genes")) %>% # use across to calculate # of genes per taxon in each OG and write the output as new columns
  mutate(across(ends_with("_total_genes"), ~ ifelse(is.na(.x), 0, .x))) # go through all of the newly created columns and convert NAs to zeroes
...
orthogroups_psam <- 
  ...
  as.data.frame() %>% 
  clean_names() %>% 
  mutate(psam_gene_count = str_count(psammoneis_japonica, ',')+1) # add this to check that we have the correct total for psam coding genes
orthogroups_psam %>% 
  summarise(psam_gene_total = sum(psam_gene_count)) # check that we have 15170 genes in total
