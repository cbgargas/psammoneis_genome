...
```{r}
#| label: Crsip HGT index classification
...
donor_directory <-
  "/Users/cbg/01_projects/psammoneis_genome_and_bacteria/data/02_hgt_index/HGT_index_output2/HGT_index_output_donor/"
recipient_directory <-
  "/Users/cbg/01_projects/psammoneis_genome_and_bacteria/data/02_hgt_index/HGT_index_output2/HGT_index_output_recipient/"
...
# Number of unique orthogroups per domain assignment
hgt_index_output_orth %>%
  filter(grepl("^Psammoneisjap", qseqid)) %>%
  group_by(domain_assignment) %>% 
...
# Domain assignments for prokaryotic hits
    hgt_index_output_orth %>%
  filter(grepl("^Psammoneisjap", qseqid)) %>%
  filter(domain_assignment == "prokaryote_strong" | domain_assignment == "prokaryote") %>%
...
# class assignments
  ...
  filter(grepl("^Psammoneisjap", qseqid)) %>%
  filter(domain_assignment == "prokaryote_strong" | domain_assignment == "prokaryote") %>%
  filter(bac_genome == "0F" | bac_genome == "1F" | bac_genome == "2F"| bac_genome == "3F") %>%
  select(where(~ !(all(is.na(.)) | all(. == "")))) %>% # Drop empty columns
  select(hgt_class) %>%
    table(useNA = "ifany") 
...
psam_only_orths <-  
  orthogroups %>% 
  filter(if_all(orthogroups_psam_cols, ~ is.na(.))) %>% 
...
# Then we will import Psammoneis CDS sequences from orthofinder and filter them to only the sequences of Class A hits to our nbacterial genome
...
write.table(psam_CDS_classA_bacgen,
  sep = "\n",
  file = "~/01_projects/psammoneis_genome_and_bacteria/data/02_hgt_index/orthofinder_psammoneis_wade/Psammoneis_japonica_classA_bacGenHits.faa",
  quote = FALSE, row.names = FALSE, col.names = FALSE
)

# read in our csv files from our blast searches
description_files <-
  list.files(path = "/Users/cbg/01_projects/psammoneis_genome_and_bacteria/data/02_hgt_index/orthofinder_psammoneis_wade/classA_bacGen_blastp/", full.names = T, pattern = "^HHR9STDN01R")
...
descriptions <-
  ...
  left_join(., 
            vroom::vroom("/Users/cbg/01_projects/psammoneis_genome_and_bacteria/data/02_hgt_index/orthofinder_psammoneis_wade/classA_bacGen_blastp/HHRUG9Y2013-Alignment-HitTable.csv", col_names = FALSE), 
            by = c("Accession"="X2")) %>% 
   group_by(X1) %>% # group our data by sqeqid
  top_n(1, X4) %>% # select the top blast search result by bitscore for each seq_id
  slice(1) %>%
  # mutate(orthogroup = str_remove(orthogroup, ".fa.donor.dmndBlastOut.tsv")) %>%
  ungroup()
