...
pacman::p_load(report, skimr, janitor, glue, ggridges, ggdist, ggtext, ggside, GenomicRanges, here, reshape2, tidyverse, patchwork, seqinr, coRdon, data.table, TidyDensity)
# pacman::p_load(report, skimr, janitor, glue, ggridges, ggdist, ggtext, ggside, GenomicRanges, here, reshape2, tidyverse, tidylog, patchwork, seqinr, coRdon, data.table, TidyDensity)
```
...
colnames_donor <- c("qseqid", "sseqid_d", "pident_d", "length_d", "mismatch_d", "gapopen_d", "qstart_d", "qend_d", "sstart_d", "send_d", "evalue_d", "bitscore_d")
colnames_recipients <- c("qseqid", "sseqid_r", "pident_r", "length_r", "mismatch_r", "gapopen_r", "qstart_r", "qend_r", "sstart_r", "send_r", "evalue_r", "bitscore_r")

ORF_headers <-
  read_delim(file = here("data","01_psam_intergenic_HGT_index","psam_genome","orfipy_psamm_complement_intergenic.fa_out","psamm_complement_ORFs_orfipy_headers.delim"), delim = " ", col_names = c("qseqid", "coords_strand", "type", "length", "frame", "start", "stop")) %>%
  mutate(across(
    .cols = everything(),
    .fns = ~ str_remove(
      string = .x,
...
euk <-
  read_tsv(file = here("data","01_psam_intergenic_HGT_index","DMNDblastp_uniprot_output_psam","dmnd_blastp_hgt_output","intergenic_dmnd_uniprot_euk.out.tsv"), col_names = colnames_recipients, progress = T) %>%
...
prok <-
  read_tsv(file = here("data","01_psam_intergenic_HGT_index","DMNDblastp_uniprot_output_psam","dmnd_blastp_hgt_output","intergenic_dmnd_uniprot_prok.out.tsv"), col_names = colnames_donor, progress = T) %>%
  group_by(qseqid) %>% # group our data by sqeqid
...
file3_uniprot <-
  file3 %>%
  separate(qseqid, sep = ":", into = c("chrm", "length_orf"), remove = F) %>% # split ORF names
  separate(length_orf, sep = "_", into = c("length_orf", "orf_id"), remove = T) %>% # split ORF names 2
  ...
  separate(sseqid_d, sep = "\\.", into = c("chrm_bac", "bac_gen_peg"), remove = T) %>% # bacterial genome hits
  mutate(length_orf = str_replace_all(string = length_orf, pattern = "[/(/)$]", replacement = "")) %>% # remove () from orf length
  mutate(chrm = str_replace_all(string = chrm, pattern = "_quiver_pilon$", replacement = "")) %>% # remove quiver pilon from chrm
  ...
  left_join(., ORF_headers, by = c("qseqid", "chrm", "length_orf", "orf_id")) %>% # join ORF information
...
remove_these <-
  (file3_uniprot %>%
    select(chrm_bac) %>%
    filter(!chrm_bac %in% c("0F", "1F", "2F", "3F")) %>%
...
file3_uniprot_prok_hits_eco <-
  read_tsv(
    file = "/Users/cbg/01_projects/psammoneis_genome_and_bacteria/data/01_psam_intergenic_HGT_index/file3_uniprot_prok_hits_ecology_domain.tsv",
    col_names = c("organism_uniprot_d", "ecology_broad", "ecology_fine", "domain"), skip = 1
  ) %>%
  unique() %>%
  left_join(
    (file3_uniprot %>%
...
file3_uniprot_prok_hits_0Fthru3F <-
  file3_uniprot_prok_hits_eco %>% 
  filter(!is.na(chrm_bac)) %>% 
  mutate(header = paste(chrm_bac, length_bac, sep = ":"))

# here we will have an unsliced version of the above so that we can isolate all of the best prokarayote hits to our porkaryote/prokaryote_strong h index classifications.
# once we have this, we will download these nuclotide sequences and align them with our qseqid.
prok_unSliced <-
  read_tsv(file = "/Users/cbg/01_projects/psammoneis_genome_and_bacteria/data/01_psam_intergenic_HGT_index/DMNDblastp_uniprot_output_psam/dmnd_blastp_hgt_output/intergenic_dmnd_uniprot_prok.out.tsv", col_names = colnames_donor, progress = T) %>%
  group_by(qseqid) %>% # group our data by sqeqid
  dplyr::filter(qseqid %in% file3_uniprot_prok_hits_eco$qseqid) %>%
  ungroup() %>%
  separate(sseqid_d, sep = "\\|", into = c("uniprot_prefix_d", "uniprot_id_d", "product_d"), remove = F) %>% # uniprot hits
  separate(sseqid_d, sep = ":", into = c("chrm_bac", "length_bac"), remove = T) %>% # bacterial genome hits
  select(-c(starts_with("uniprot_prefix"))) %>%
  select(uniprot_id_d) %>%
...
# write it!
write_delim(prok_unSliced, file = "/Users/cbg/01_projects/psammoneis_genome_and_bacteria/data/file3_uniprot_prok_IDs_all_hits.delim", delim = " ", col_names = FALSE)

# read in out output from uniprot search
uniprot2ncbi_ids_all_hits <-
  read_tsv(file = "/Users/cbg/01_projects/psammoneis_genome_and_bacteria/data/uniprot-yourlist_M20220411A084FC58F6BBA219896F365D15F2EB444998D7P.tab", col_names = TRUE, col_types = NULL) %>%
  janitor::clean_names() %>%
  left_join(
    (read_tsv(file = "/Users/cbg/01_projects/psammoneis_genome_and_bacteria/data/01_psam_intergenic_HGT_index/DMNDblastp_uniprot_output_psam/dmnd_blastp_hgt_output/intergenic_dmnd_uniprot_prok.out.tsv", col_names = colnames_donor, progress = T) %>%
      group_by(qseqid) %>% # group our data by sqeqid
      dplyr::filter(qseqid %in% file3_uniprot_prok_hits_eco$qseqid) %>%
      ungroup() %>%
      separate(sseqid_d, sep = "\\|", into = c("uniprot_prefix_d", "uniprot_id_d", "product_d"), remove = T) %>% # uniprot hits
      select(-c(starts_with("uniprot_prefix")))),
    .,
    by = c("uniprot_id_d" = "entry")
  ) %>%
  mutate(cross_reference_ref_seq = str_extract(cross_reference_ref_seq, "WP_[:digit:]+.1")) %>%
  rename(sseqid = cross_reference_ref_seq) %>%
  dplyr::select(-c(entry_name, uniprot_id_d, product_d)) %>%
  drop_na() %>%
  relocate(sseqid, .after = qseqid)

write_tsv(uniprot2ncbi_ids_all_hits, file = "/Users/cbg/01_projects/psammoneis_genome_and_bacteria/data/uniprot2ncbi_ids_all_hits.tsv", col_names = FALSE)

rm(euk, prok, ORF_headers, uniprot_reviewed, file3, colnames_donor, colnames_recipients, remove_these, bac_gff_directory, bac_gff_files)
```

```{r}
#| label: hgt index data summaries
#| include: false

#  # X is a sample of 100 normally distributed random variables
# P = ecdf(file3$h)    # P is a function giving the empirical CDF of X
# P(0.0)         # This returns the empirical CDF at zero (should be close to 0.5)
# plot(P)

file3_uniprot %>%
  select(evalue_r, evalue_d, bitscore_r, bitscore_d, h) %>%
  summary()
...
# table(file3_uniprot_prok_hits_eco$gene_ontology_molecular_function_uniprot_d) %>% 
#   as.data.frame() %>% 
#   mutate(Var1 = str_remove_all(Var1, " \\[GO\\:[:digit:]+\\]")) %>% 
#   ggplot(aes(y=Var1, x = Freq)) +
#   geom_bar(stat = "identity")
# 
# table(file3_uniprot_prok_hits_eco$gene_ontology_biological_process_uniprot_d) %>% 
#   as.data.frame() %>% 
#   mutate(Var1 = str_remove_all(Var1, " \\[GO\\:[:digit:]+\\]")) %>% 
#   ggplot(aes(y=Var1, x = Freq)) +
#   geom_bar(stat = "identity")
# 
# table(file3_uniprot_prok_hits_eco$bac_gen_gene_annotation) %>% 
#   as.data.frame() %>% 
#   add_column() %>% 
#   ggplot(aes(y=Var1, x = Freq)) +
#   geom_bar(stat = "identity")
...
file3_uniprot_prok_hits_0Fthru3F %>% 
  filter(chrm %in% c("000002F","000037F","000089F","000103F","000532F")) %>% 
  arrange(chrm) %>% 
  select(chrm, chrm_bac, bac_gen_peg) 

# Now, let's check out the ones that have multiple hits to the same bacterial genes
file3_uniprot_prok_hits_0Fthru3F %>% 
  filter(chrm %in% c("000002F","000089F","000103F","000532F")) %>% 
  arrange(chrm) %>% 
  select(chrm, chrm_bac, bac_gen_peg, qstart_d, qend_d, sstart_d, send_d, type, frame, strand, bac_gen_gene_annotation)
...
inner_join(as.data.frame(table(file3_uniprot_prok$chrm)),
  ...
  summarise(total = sum(Freq.x), sum_nonOverlap = sum(Freq.x) - sum(n_overlaps), sum_nonOverlap_1k = sum(Freq.x) - sum(n_overlaps_1k), sum_overlaps = sum(n_overlaps), sum_overlaps_1k = sum(n_overlaps_1k)) %>%
...
ORFs <-
  read.fasta(file = "/Users/cbg/01_projects/psammoneis_genome_and_bacteria/data/01_psam_intergenic_HGT_index/psam_genome/orfipy_psamm_complement_intergenic.fa_out/psamm_complement_ORFs_orfipy.fa", as.string = T, seqtype = "DNA")

psamm_cds <-
  read.fasta(file = "/Users/cbg/01_projects/psammoneis_genome_and_bacteria/data/01_psam_intergenic_HGT_index/DMNDblastp_uniprot_output_psam/01_genomes_CDS/psam_genic.fa", as.string = T, seqtype = "DNA")
...
write.table(psamm_subsampled,
  sep = "\n",
  file = "/Users/cbg/01_projects/psammoneis_genome_and_bacteria/data/01_psam_intergenic_HGT_index/sequence_ordination/psam_genic_subsampled.fa",
...
write.table(ORFs_filt[, 1:2],
  sep = "\n",
  file = "/Users/cbg/01_projects/psammoneis_genome_and_bacteria/data/01_psam_intergenic_HGT_index/sequence_ordination/psamm_prokaryote_candidate_ORFs_filtered.fa",
  quote = FALSE, row.names = FALSE, col.names = FALSE
)

# rm() objects that aren't needed
rm(ORFs, ORFs_filt, psamm_cds, psamm_subsampled)
...
# check that all of the Psammoneis genes are present 
orthogroups %>% 
  mutate(psam_gene_count = str_count(psammoneis_japonica, ',')+1) %>% 
  summarise(psam_gene_total = sum(psam_gene_count))

# read in UniProt information for each sseqid in our blast databases
uniprot_reviewed <-
  read_tsv(file = "/Users/cbg/01_projects/psammoneis_genome_and_bacteria/data/uniprot-reviewed_yes.tsv", col_names = T) %>%
  as.data.frame() %>%
  clean_names()
...
recipient_directory <-
  "/Users/cbg/01_projects/psammoneis_genome_and_bacteria/data/02_hgt_index/HGT_index_output/HGT_index_output_recipient/"
...
d_r_hOrth <-
  ...
  dplyr::summarise(dplyr::across(dplyr::starts_with("h"),
    ~ mean(.x, na.rm = TRUE),
    .names = "{.col}_orth"
...
orthogroups_psam_cols <-
  colnames(orthogroups %>% select(-psammoneis_japonica, -orthogroup))

# Orthogroups with multiple diatoms
orthogroups_diatom_only <-
  orthogroups %>% 
  select(-nannochloropsis_gaditana, -ectocarpus_siliculosus) %>% 
      filter(!if_all(colnames(orthogroups %>% select(-psammoneis_japonica, -orthogroup, -nannochloropsis_gaditana, -ectocarpus_siliculosus)), ~ is.na(.))) 
...
# Then we will import Psammoneis CDS sequences from orthofinder and filter them to only the sequences of Class A hits to our bacterial genome
...
psam_CDS <-
  bioseq::read_fasta(file = "~/01_projects/psammoneis_genome_and_bacteria/data/02_hgt_index/orthofinder_psammoneis_wade/Psammoneis_japonica.faa", type = "AA")


psam_CDS_classA_bacgen <-
  tibble(label = names(psam_CDS), sequence = psam_CDS) %>% 
  left_join(qseqs_classA_bacGen,., by = c('qseqid' = 'label')) %>% 
  rename(label = qseqid) %>% 
  mutate(label = paste(">", label, sep = ""))

#split data frame into n equal-sized data frames
psam_CDS_classA_bacgen_split <- split(psam_CDS_classA_bacgen, factor(sort(rank(row.names(psam_CDS_classA_bacgen))%%10)))
# Make a counter object so we can generate and change the number of each file as lapply iterates
counter <- 0
# split up the psam_CDS_classA_bacgen file into chunks so that ncbi BlastP doesn't throw a fit
lapply(psam_CDS_classA_bacgen_split, function(x) {
     counter <<- counter + 1; 
     write.table(x, 
                 file = paste("~/01_projects/psammoneis_genome_and_bacteria/data/02_hgt_index/orthofinder_psammoneis_wade/Psammoneis_japonica_classA_bacGenHits_", 
                              counter,
                              ".faa", 
                              sep = ""),
                          sep = "\n", 
...
# read in our csv files from our blast searches
description_files <-
  list.files(path = "/Users/cbg/01_projects/psammoneis_genome_and_bacteria/data/02_hgt_index/orthofinder_psammoneis_wade/classA_bacGen_blastp/", full.names = T, pattern = "Descriptions")
HitTable_files <-
  list.files(path = "/Users/cbg/01_projects/psammoneis_genome_and_bacteria/data/02_hgt_index/orthofinder_psammoneis_wade/classA_bacGen_blastp/", full.names = T, pattern = "HitTable")

# Here we use fread, lapply, and bind_rows to read in all of our output files and bind them by row (i.e., concatenating them) into a single dataframe for donor and recipient results, respectively.
descriptions <-
  bind_rows(lapply(description_files, fread)) %>%
  mutate(Accession = str_extract(Accession, '[:alnum:]+\\.[:digit:]""\\)$')) %>% 
  mutate(Accession = str_remove(Accession, '""\\)$')) %>% 
  left_join(., 
            (bind_rows(lapply(HitTable_files, fread)) %>% 
               mutate(V3 = round(V3, digits = 2))), 
            by = c("Accession"="V2", "Per. ident"="V3"))
...
descriptions_sliced <-
  ...
  left_join(., 
            (bind_rows(lapply(HitTable_files, fread)) %>% 
               mutate(V3 = round(V3, digits = 2))), 
            by = c("Accession"="V2", "Per. ident"="V3")) %>% 
   group_by(V1) %>% # group our data by sqeqid
  top_n(1, V12) %>% # select the top blast search result by bitscore for each seq_id
  slice(1) %>%
  # mutate(orthogroup = str_remove(orthogroup, ".fa.donor.dmndBlastOut.tsv")) %>%
  ungroup()

# let's create a visualization for how many of our Class A hits to Psammoneis are hitting to Bacteria when we use NCBI blastp
descriptions_sliced %>% 
  select("Scientific Name") %>% 
  mutate(Scientific.Name = str_extract(`Scientific Name`, "[:alpha:]+ [:alpha:]+")) %>% 
  table() %>% 
  as.data.frame() %>% 
  mutate(highlight_flag = str_detect(Scientific.Name, "bacterium$|Blastopirellula|group")) %>% 
  ggplot() +
  geom_bar(
    aes(
      y = forcats::fct_rev(Scientific.Name), 
      x = Freq,
      fill = highlight_flag), 
    stat = "identity", 
    width = .6) +
  scale_fill_manual(name = "Bacteria", values=c("grey50","purple")) +
  scale_x_continuous(limits = c(0,16), breaks = c(seq(0, 16, by = 2)), expand = c(0,0)) +
  # scale_y_discrete(labels = scales::wrap_format(10)) +
  xlab("No. of hits (to 106 Class A sequences)") +
  ylab("NCBI BlastP top hit") +
  theme(
    axis.text.x = element_text(angle = 0, hjust = .5, color = "#000000"),
...
# Next, let's see which of our bacterial hits are shared between Matt's results and mine
descriptions_sliced %>% 
  mutate(Scientific.Name = str_extract(`Scientific Name`, "[:alpha:]+ [:alpha:]+")) %>% 
  mutate(highlight_flag = str_detect(Scientific.Name, "bacterium$|Blastopirellula|group")) %>% 
  left_join(
...
# finally, let's see how it goes for all of our other "Bacterial" hits
descriptions_sliced %>% 
  mutate(Scientific.Name = str_extract(`Scientific Name`, "[:alpha:]+ [:alpha:]+")) %>% 
  mutate(highlight_flag = str_detect(Scientific.Name, "bacterium$|Blastopirellula|group")) %>% 
  left_join(
    ...
    ., 
    by = c("qseqid" = "V1")) %>% 
  filter(highlight_flag == TRUE) %>% 
  select(qseqid, sseqid_d, organism_uniprot_d, Scientific.Name, highlight_flag)
```
...
Here we use blobtools to generate blob plots from our original Illumina and PacBio read files.
