...
pkgs <- c("report", "skimr", "janitor", "glue", "ggridges", "ggdist", "ggtext", "ggside", "GenomicRanges", "here", "reshape2", "tidyverse", "patchwork", "seqinr", "coRdon", "data.table", "TidyDensity", "arrow", "RSQLite") # Names of packages we want to load
pkgs_tidylog <- c("report", "skimr", "janitor", "glue", "ggridges", "ggdist", "ggtext", "ggside", "GenomicRanges", "here", "reshape2", "tidyverse", "patchwork", "seqinr", "coRdon", "data.table", "TidyDensity", "arrow", "RSQLite", "tidylog") # same as above, but with tidylog if we want more verbose output when using tidy.

pacman::p_load(char = pkgs, install = TRUE, character.only = TRUE) # Load our pacakges
...
blastp_results <-
  read_tsv(
    ...
    progress = T) %>% 
  mutate(acc.ver = str_remove_all(sseqid, paste(c("^[:alpha:]+\\|+","\\|$"), collapse="|"))) 
...
# looking at our group composition for each qseqid and it's blastp hits
  blastp_results_joined %>% 
    group_by(qseqid, category_expanded) %>% 
    select(qseqid, category_expanded) %>% 
    table() %>% 
    as.data.frame() %>% 
    pivot_wider(names_from = category_expanded, values_from = Freq) %>% 
    mutate( # no we make a column to calculate the percentage of hits to each qseqid that are bacgterial/archaeal
    perc_arc = Archaea/rowSums(select(., contains(c("Arc","Bac","Euk")))),
    perc_bac = Bacteria/rowSums(select(., contains(c("Arc","Bac","Euk")))),
    perc_euk = Eukaryota/rowSums(select(., contains(c("Arc","Bac","Euk"))))
    ) %>% 
    select(-c(Archaea, Bacteria, Eukaryota)) %>% 
    rename(
      Archaea = perc_arc,
      Bacteria = perc_bac,
      Eukaryota = perc_euk) %>% 
    ...
    ggplot(aes(x = percentage, y = forcats::fct_rev(percent_groups), fill = percent_groups)) +
    stat_slab(scale = 0.7, normalize = "groups") +
    stat_halfeye(stroke = 1, point_color = "grey50", interval_color = "grey75", side = "top", scale = 0.7, slab_size = NA, layout = "swarm") +
    viridis::scale_fill_viridis(discrete=T) +
    scale_x_continuous(limits = c(0,1), breaks = c(seq(0, 1, by = .25))) +
    xlab("Percentage") +
    ylab("") +
    theme(
      axis.text.x = element_text(angle = 0, hjust = .5, color = "#000000"),
...
df <-
  ...
  dplyr::slice(1) %>%
  ungroup() %>% 
  left_join(., hgt_index_output_orth_prok_psam, by = "qseqid") %>% 
  filter(chrm_bac %in% c("0F","1F","2F","3F")) %>% 
  left_join(., 
            (blastp_results_joined %>% 
    group_by(qseqid, category_expanded) %>% 
    ...
    pivot_wider(names_from = category_expanded, values_from = Freq) %>% 
    mutate( # no we make a column to calculate the percentage of hits to each qseqid that are bacgterial/archaeal
    perc_bac = Bacteria/rowSums(select(., contains(c("Bacteria","Eukaryota")))),
    perc_euk = Eukaryota/rowSums(select(., contains(c("Bacteria","Eukaryota"))))
    ) %>% 
    ...
    by = "qseqid") %>% 
    pivot_longer(
      cols = contains(c("Bacteria","Eukaryota")),
      names_to = "percent_groups",
      values_to = "percentage",
      values_drop_na = TRUE
      ) 
...
blastp_results_joined %>% 
  select("category_expanded") %>% 
  table() %>% 
  as.data.frame() %>% 
  ggplot(aes(
      y = forcats::fct_rev(category_expanded), 
      x = Freq)) +
  geom_col(fill = "#360c5b") +
  scale_x_continuous(breaks = c(seq(0, 4250, by = 250), 100), expand = c(0,0)) +
  # coord_cartesian(xlim=c(0, 1000)) +
  geom_text(aes(label = Freq), nudge_x = c(100, 150,-200), colour = c("black", "black", "white")) + 
  # scale_y_discrete(labels = scales::wrap_format(10)) +
  xlab("No. of hits (to 106 Class A sequences)") +
  ylab("NCBI BlastP hits") +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1, color = "#000000", face = "bold"),
...
blastp_results_joined %>% 
  ...
  ggplot(aes(
      ...
      x = Freq)) +
  geom_col(fill = "#360c5b") +
  scale_x_continuous(limits = c(0,450), breaks = c(seq(0, 450, by = 50)), expand = c(0,0)) +
  geom_text(aes(label = Freq), nudge_x = c(5, 8, -20), colour = c("black", "black", "white")) + 
  # scale_y_discrete(labels = scales::wrap_format(10)) +
  xlab("No. of hits (to all Class A sequences)") +
  ylab("NCBI BlastP hits") +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1, color = "#000000", face = "bold"),
    axis.text.y = element_text(color = "#000000", size = 12),
...
blastp_results_joined %>% 
  ...
  ggplot(aes(
      ...
      x = Freq)) +
  geom_col(fill = "#360c5b") +
  scale_x_continuous(limits = c(0,100), breaks = c(seq(0, 100, by = 25)), expand = c(0,0)) +
  geom_text(aes(label = Freq), nudge_x = c(3, 3), colour = c("black", "black")) + 
  # scale_y_discrete(labels = scales::wrap_format(10)) +
  xlab("No. of 0F-3 Fhits (Class A sequences)") +
  ylab("NCBI BlastP hits") +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1, color = "#000000", face = "bold"),
    axis.text.y = element_text(color = "#000000", size = 12),
...
# finally, let's see how it goes for all of our other "class A" hits
df <- 
  left_join(
      (hgt_index_output_orth %>%
         filter(grepl("^Psammoneisjap", qseqid)) %>%
      ...
      by = "qseqid") %>% 
  select(qseqid, sseqid_d, organism_uniprot_d, category_expanded)
