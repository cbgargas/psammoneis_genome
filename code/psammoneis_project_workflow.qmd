...
First, we use the SLURM script 'submit_01_download_ncbi_nr_db.slurm' to download the most recent NR blast database and taxonomy database. 
...
psam_qseqs_classA_seqs <-
  ...
  left_join(qseqs_classA_NONbacGen,., by = c('qseqid' = 'label')) %>% 
  rename(label = qseqid) %>% 
...
We extract the contents of the .gz files and join them using the following one-liner in the terminal. Because I am using zsh, We use the (n) to sort our files numerically.  

```
head -n1 prot.accession2taxid.FULL.1 > prot.accession2taxid.FULL.2022-10-09.tsv; for fname in prot.accession2taxid.FULL.*; do tail -n+2 $fname >> prot.accession2taxid.FULL.2022-10-09.tsv; done
```
...
# I created the following sqlite db using the sqlite file browser
...
# Below I have set up some functions to time how long each one takes to parse our data and perform functions on it. 
