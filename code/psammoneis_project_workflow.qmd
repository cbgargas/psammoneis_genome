...
genic_mRNA_TE_density_filtered <- 
  genic_mRNA_TE_density %>% 
  mutate(gene_name = paste("Psammoneisjap_",gene_name, sep = "")) %>% 
  filter(gene_name %in% blastp_results_final$qseqid) %>% 
  group_by(gene_name) %>% 
...
intergenic_TE_density_filtered <- 
  intergenic_TE_density %>% 
  filter(gene_name %in% file3_uniprot_prok_hits_0Fthru3F$qseqid)
  
intergenic_TE_density_filtered %>% 
  select(TE_density) %>% 
  table()

# Now we subsample each of our TE density data-frames for genic and intergenic TEs, and replicate 500x to plot the density of our averages
# 
vlines_ig <- 
  as.data.frame(table(intergenic_TE_density_filtered$TE_density)) %>% 
  mutate(xint = as.numeric(as.character(Var1)))

replicate(500,
          mean((genic_mRNA_TE_density %>% 
                  slice_sample(n = 40, replace = TRUE))$TE_density)
          ) %>% 
  as.data.frame() %>% 
  ggplot(aes(x = .)) +
  geom_density() +
  geom_vline(data = vlines_ig, 
             aes(xintercept = xint, colour = Freq), 
             linetype = "longdash") +
  viridis::scale_color_viridis(discrete=F, begin = 0.15, end = 1, direction = 1) +
  theme(
...
vlines_g <- 
  as.data.frame(table(genic_mRNA_TE_density_filtered$TE_density)) %>% 
  mutate(xint = as.numeric(as.character(Var1)))

replicate(500,
          mean((genic_mRNA_TE_density %>% 
                  slice_sample(n = 17, replace = TRUE))$TE_density)
          ) %>% 
  as.data.frame() %>% 
  ggplot(aes(x = .)) +
  geom_density() +
  geom_vline(data = vlines_g, 
             aes(xintercept = xint, colour = Freq), 
             linetype = "longdash") +
  viridis::scale_color_viridis(discrete=F, begin = 0.3, end = 1, direction = 1) +
  theme(
    text = element_text(size = 12, color = "black"),
    # Hide panel borders and remove grid lines
    panel.border = element_blank(),
...
# now let's do a t-test vs our actual values
# calculate the observed statistic
observed_mean_genic <- 
  genic_mRNA_TE_density %>%
  specify(response = TE_density) %>%
  calculate(stat = "mean")

# generate the null distribution
null_dist_1_sample <- genic_mRNA_TE_density_filtered %>%
  specify(response = TE_density) %>%
